...
#define DASHA
        ...
        // Use bit masks to check for duplicates
        for (int i = 0; i < 9; i++) {
            unsigned short rowMask = 0;
            unsigned short colMask = 0;
            for (int j = 0; j < 9; j++) {
                // Check rows
                if (field[i][j] != 0) {
                    if (rowMask & (1 << field[i][j])) {
                        score++;
                    } else {
                        rowMask |= (1 << field[i][j]);
                    }
        ...
        for (int block = 0; block < 9; block++) {
            unsigned short blockMask = 0;
            int rowOffset = (block / 3) * 3;
            int colOffset = (block % 3) * 3;
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    int val = field[rowOffset + i][colOffset + j];
                    if (val != 0) {
                        if (blockMask & (1 << val)) {
                            score++;
                        } else {
        ...
        for (std::size_t i = 0; i < SUDOKU_SIZE; ++i) {
            ...
            for (std::size_t j = 0; j < SUDOKU_SIZE; ++j) {
                if (initial_field[i][j] != 0)
                    continue;
                auto digit = field[i][j];
                if (digits_by_rows[i][digit] || locked_digits_by_rows[i][digit])
                    error++;
            }
            // by columns
            for (std::size_t j = 0; j < SUDOKU_SIZE; ++j) {
                if (initial_field[i][j] != 0)
                    continue;
                auto digit = field[i][j];
                if (digits_by_columns[i][digit] || locked_digits_by_columns[i][digit])
                    error++;
            }
            // by subgrids
            for (std::size_t j = 0; j < SUDOKU_SIZE; ++j) {
                if (initial_field[i][j] != 0)
                    continue;
                auto digit = field[i][j];
                if (digits_by_subgrids[i][digit] || locked_digits_by_subgrids[i][digit])
                    error++;
            }
        }
